...
export const exportAuthorAsync = createAsyncThunk("api/exportAuthorAsync",async(entity)=>{
    const res = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/Author/ExportData`,entity)
    return await res.data
})
...
export const authorSlice = createSlice({
    ...
    reducers:{
        removeXmlItem:(state,action)=>{
            ...
            const filtered = state.xmlItems.filter((e)=>e.firstName!==xmlItem.firstName&&e.lastName!==xmlItem.lastName)
    ...
    extraReducers:{
        ...
        [exportAuthorAsync.fulfilled]:(state,action)=>{
