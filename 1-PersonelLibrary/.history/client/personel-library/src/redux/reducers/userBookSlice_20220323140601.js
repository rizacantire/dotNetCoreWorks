...
export const getUserBooksByUserIdAsync = createAsyncThunk("api/UserBooks/getUserBooksByUserId",async(id)=>{
    const res = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/UserBooks/${id}`,{headers:AuthHeader()})
    return await res.data.result;
})
...
export const deleteUserBookAsync = createAsyncThunk("api/deleteUserBook",async(data)=>{
    console.log(data);
    const res = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/UserBooks/delete`,data,{headers:AuthHeader()})
    return res.data
...
export const userBookSlice = createSlice({
    ...
    initialState:{
        ...
        [getUserBooksByUserIdAsync.fulfilled]:(state,action)=>{
        ...
        [updateUserBookAsync.fulfilled]:(state,action)=>{
            const {id,finishReadDate} = action.meta.arg
            const index = state.items.findIndex((item)=>item.id === id)
            state.userBooks[index].finishReadDate = finishReadDate
        ...
        [deleteUserBookAsync.fulfilled]:(state,action)=>{
            //state.items.pop(action.payload)
            const {id} = action.meta.arg
            const index = state.items.findIndex((item)=>item.id === id)
            state.items.pop(state.items[index])
            state.isUpdated = true;            
            console.log(action);
        },
        [deleteUserBookAsync.pending]: (state, action) => {
            state.isUpdated = false;
        },
        [deleteUserBookAsync.rejected]: (state, action) => {
            state.isUpdated = false;
...
export const userBooksByUserId = (state)=>state.userBooks.userBooks;
