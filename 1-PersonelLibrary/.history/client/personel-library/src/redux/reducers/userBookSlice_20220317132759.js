...
export const getUserBooksAsync = createAsyncThunk("api/UserBooks/getByUser",async()=>{
    const res = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/UserBooks/getByUser`,{headers:AuthHeader()})
    return await res.data;
})

export const addUserBookAsync = createAsyncThunk("api/addUserBook",async(data)=>{
    const res = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/api/UserBooks`,data,{headers:AuthHeader()})
    return res.data
})
