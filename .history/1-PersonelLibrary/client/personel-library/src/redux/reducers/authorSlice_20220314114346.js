...
export const addAuthorAsync =  createAsyncThunk("api/AddAuthor",async(data)=>{
    const response = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/Author`,data)
    return await response.data
})
export const deleteAuthorAsync =  createAsyncThunk("api/AddAuthor",async(data)=>{
    const response = await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/Author?id=`+data)
    return await response.data
})
